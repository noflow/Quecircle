...
function StudioPage() {
  const [section, setSection] = useState<"overview" | "members" | "editorial">("overview");
  return <section className="page studio-workspace"><div className="studio-workspace-head"><div><p className="eyebrow">PRIVATE CINEAPE STUDIO</p><h1>Run the site behind the scenes.</h1><p>Monitor members and publish the official CineApe point of view.</p></div><div className="tabs studio-tabs"><button className={section === "overview" ? "chosen" : ""} onClick={() => setSection("overview")}>Overview</button><button className={section === "members" ? "chosen" : ""} onClick={() => setSection("members")}>Members</button><button className={section === "editorial" ? "chosen" : ""} onClick={() => setSection("editorial")}>Editorial desk</button></div></div>{section === "overview" && <StudioPageOverview />}{section === "members" && <StudioMembers />}{section === "editorial" && <StudioEditorial />}</section>;
}
